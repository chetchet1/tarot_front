...
const handleCardClick = async (index: number) => {
  if (props.cards[index]) {
    ...
    if (userStore.isPremium) {
      console.log('[CupOfRelationship] 프리미엄 사용자 카드 클릭:', {
        index,
      ...
      if (showPositionMeaning.value && selectedPosition.value === index + 1) {
        ...
        console.log('[CupOfRelationship] 포지션 의미 숨김');
      } else {
        selectedPosition.value = index + 1;
        showPositionMeaning.value = true;
        console.log('[CupOfRelationship] 포지션 의미 표시:', selectedPosition.value);
      }
    }
  }
};
...
// 디버깅용 생명주기 훅
onMounted(() => {
  console.log('[CupOfRelationship] 컴포넌트 마운트됨:', {
    isPremium: userStore.isPremium,
...
watch(showPositionMeaning, (newValue, oldValue) => {
  console.log('[CupOfRelationship] showPositionMeaning 변경:', {
    old: oldValue,
    new: newValue,
  ...
  setTimeout(() => {
    ...
    console.log('[CupOfRelationship] DOM 체크:', {

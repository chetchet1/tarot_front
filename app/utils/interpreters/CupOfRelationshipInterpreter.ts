...
export class CupOfRelationshipInterpreter {
  ...
  public generateStructuredPromptForAI(cards: any[], topic: string, customQuestion?: string): string {
    let prompt = '당신은 20년 경력의 전문 타로 리더입니다. 컵 오브 릴레이션십 배열법은 **관계와 연애를 전문적으로 다루는 배열**입니다.\n\n';
    
    // 컵 오브 릴레이션십은 본질적으로 연애/관계 전용 배열법
    prompt += '⚠️ 중요 지침:\n';
    prompt += '1. 컵 오브 릴레이션십은 **연애와 관계에 특화된 배열법**입니다. 모든 해석을 연애/관계 관점에서 해주세요.\n';
    prompt += '2. 질문자의 현재 상황을 고려하세요:\n';
    prompt += '   - **솔로인 경우**: 새로운 만남의 가능성, 언제 연인이 나타날지, 연애 준비 상태 등\n';
    prompt += '   - **짝사랑 중인 경우**: 상대의 마음, 가능성, 접근 방법 등\n';
    prompt += '   - **연인이 있는 경우**: 현재 관계 상태, 미래 전망, 개선점 등\n';
    prompt += '   - **이별/재회를 고민하는 경우**: 관계 회복 가능성, 새 출발의 필요성 등\n';
    prompt += '3. **객관적이고 현실적인 해석**을 하세요. 카드가 부정적이면 솔직하게 어려움을 전달하세요.\n';
    prompt += '4. 무조건적인 희망보다는 **실질적이고 행동 가능한 조언**을 제공하세요.\n';
    prompt += '5. 역방향 카드가 많다면 관계의 어려움을 인정하고, 구체적인 해결책을 제시하세요.\n\n';
    
    if (topic === 'love' || topic === '연애') {
      prompt += '💘 **연애운 해석 강화**: 이 리딩은 연애운에 초점을 맞춰주세요.\n\n';
    } else {
      // 다른 주제라도 컵 오브 릴레이션십은 관계 중심으로 해석
      prompt += `💘 비록 '${topic}'에 대한 질문이지만, 컵 오브 릴레이션십은 관계의 관점에서 해석합니다. ${topic}과 관련된 인간관계와 연결하여 해석해주세요.\n\n`;
    }
    ...
    prompt += '\n다음 형식으로 해석해주세요:\n';
    prompt += '👥 **관계의 역학** (나와 상대의 현재 상태)\n';
    prompt += '💖 **감정의 흐름** (서로의 진짜 마음)\n';
    prompt += '✨ **관계의 강점** (희망적인 요소들)\n';
    prompt += '⚠️ **극복해야 할 도전** (현실적인 문제점들)\n';
    prompt += '🔮 **미래 전망** (3개월, 6개월, 1년 후)\n';
    prompt += '🌈 **조화 수준** (두 사람의 궠형 상태)\n';
    prompt += '💡 **조언과 지침** (구체적이고 실행 가능한)\n';
  ...
  public validateAIResponse(response: string): string {
    ...
    // 필요한 섹션이 포함되어 있는지 확인
    const requiredSections = ['관계', '감정', '강점', '도전', '조언'];

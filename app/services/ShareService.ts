...
export class ShareService {
  ...
  async createDailyCardShareLink(dailyCard: any): Promise<string> {
    try {
      ...
      console.log('  - interpretation 내용:', dailyCard.interpretation);
      
      // basic_interpretation에는 JSON 형식 그대로 저장
      // SharedReading.vue에서 이를 파싱해서 표시
      let interpretationData = null;
      if (dailyCard.interpretation) {
        // interpretation 객체를 JSON 문자열로 저장
      ...
      const shareData = {
        ...
        // ISO 날짜 형식 사용 (SharedReading.vue에서 쉽게 파싱하기 위해)
        custom_question: `${new Date().toISOString().split('T')[0]} 오늘의 카드`,
        basic_interpretation: interpretationData, // JSON 문자열로 저장
        ai_interpretation: null, // AI 해석은 사용하지 않음
        shared_by: (await supabase.auth.getUser()).data?.user?.id || null
      };

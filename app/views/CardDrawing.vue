  ...
  'special-layout-mode': hasSpecialLayout
    ...
    <div class="container">
      ...
      <div class="cards-container" v-if="isComplete && drawnCards.length > 0">
        ...
        <!-- ?¼ë°˜ ì¹´ë“œ ?ˆì´?„ì›ƒ -->
        <div v-else>
          <p class="instruction">?¹ì‹ ??ì¹´ë“œ?…ë‹ˆ??/p>
...
// ?¨ì? ?œê°„ ?¬ë§·??const formatExpiryTime = () => {
  ...
  if (hours > 0) {
    return `${hours}?œê°„ ${minutes}ë¶??¨ìŒ`;
  } else {
    return `${minutes}ë¶??¨ìŒ`;
  }
};
...
// ?¤í”„?ˆë“œ ?œì‹œ ?´ë¦„ ê°€?¸ì˜¤ê¸?const getSpreadDisplayName = () => {
  if (isCelticCross.value) return 'ì¼ˆí‹± ?¬ë¡œ??;
  if (isSevenStar.value) return '?¸ë¸ ?¤í?';
  if (isCupOfRelationship.value) return 'ì»??¤ë¸Œ ë¦´ë ˆ?´ì…˜??;
  return '';
};

// ì¹´ë“œ ë½‘ê¸° ë²„íŠ¼ ?ìŠ¤??const getDrawButtonText = () => {
  // ë¬´ë£Œ ?¬ìš©?ë„ ê´‘ê³  ?œì²­?¼ë¡œ ë¬´ì œ??ê°€??  return 'ì¹´ë“œ ë½‘ê¸°';
};

// ì¹´ë“œ ?´ë?ì§€ URL ?ì„± - ?µí•© ?¨ìˆ˜ ?¬ìš©
const getCardImageUrl = (card: any) => getUnifiedCardImagePath(card);

// ì¹´ë“œ ê°œìˆ˜ ê°€?¸ì˜¤ê¸?const getCardCount = () => {
  return tarotStore.selectedSpread?.cardCount || 1;
};

// ?”ë²„ê·¸ìš© ?´ë¦­ ?¸ë“¤??const debugClick = () => {
  console.log('[CardDrawing] ?”ë©´ ?´ë¦­??);
  ...
  console.log('[CardDrawing] ?„ì¬ URL:', window.location.pathname);
};
